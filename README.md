...
*  `map-reduce`  : This is what you choose when you have small workloads that don't deserve a real thread. What it does for you is to partition the data as equally as possible in n chunks (where n is configurable but defaults to cpu-no+2) and assign a thread to each chunk. Some people have reported very good results with this. Pretty basic but useful. 
