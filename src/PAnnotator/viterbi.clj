...
(defn forward [hmm alphas obs]
  (mapv (fn [state1]
         (* (reduce (fn [sum state2]
                      (+ sum (* (get alphas (.indexOf (:states hmm) state2)) (get-in (:state-transitions hmm) [state2 state1]))))
                    0
                    (:states hmm))
...
(defn delta-max [hmm deltas obs]
 (mapv (fn [state1]
        (* (apply max (map (fn [state2]
                              (* (get deltas (.indexOf (:states hmm) state2))
...
(let [observs (:observations hmm)]
  (loop [obs (next observs)
         alphas (init-alphas hmm (first observs)) ;;SORTED
         deltas alphas
         paths []]
    (if (empty? obs)
      [(paths->states (backtrack paths deltas) (:states hmm)) (float (reduce + alphas))]
      (recur (next obs)
             (forward hmm alphas (first obs))  ;;SORTED
