...
;; Globals
(def ^:dynamic *K* "number of tag states" nil )
(def ^:dynamic *vocab* "seq of word infos" nil )
(def ^:dynamic *outfile* "where to write each iteration word assignments" nil)
...
(defn pos-tag  
  ...
  (let [;[infile outfile num-iters K alpha beta] args        
    ...
    (binding [*K* K *vocab* (build-vocab (sents-fn)) *outfile* outfile]
       (learn iters alpha  beta))))
